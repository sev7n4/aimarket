...
export function StudioDock({ mode, onModeChange, children }: StudioDockProps) {
  ...
  return (
    <div
      className="absolute inset-x-0 bottom-0 z-40 flex justify-center px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] sm:px-3"
      aria-label="创作 Dock"
      data-dock-mode={mode}
    >
      <div
...
export function studioDockCanvasPadding(mode: StudioDockMode): string {
  switch (mode) {
    case "focus":
      return "pb-14";
    case "expanded":
      return "pb-[min(38vh,320px)]";
    default:
      return "pb-24 sm:pb-28";
  }
}
