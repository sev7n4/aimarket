...
interface SimpleWorkspaceSelectorProps {
  onWorkspaceChange?: (workspaceId: string) => void;
}

export function SimpleWorkspaceSelector({ onWorkspaceChange }: SimpleWorkspaceSelectorProps) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (next) {
        ...
        const ws = workspaces.find((w) => w.id === next);
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full z-50 mt-1 rounded-xl border border-white/10 bg-[#0b0b0b] p-2 shadow-xl">
          <p className="mb-1.5 px-2 text-[10px] font-medium uppercase tracking-wider text-zinc-600">
            工作空间
          </p>
