    ...
    return (
        <div className="flex h-full w-full flex-col overflow-hidden pt-8">
            ...
            {isEditingContent ? (
                <textarea
                    ref={textareaRef}
                    className="h-full w-full resize-none bg-transparent pl-4 pr-14 pt-0 pb-4 font-mono outline-none select-text"
                    style={textStyle}
        ...
        <BatchFrame batchCount={isBatchRoot ? batchCount : 0} batchExpanded={batchExpanded} batchOpening={batchOpening} batchRecovering={batchRecovering} onToggleBatch={onToggleBatch}>
            ...
            {isBatchRoot ? (
                ...
                >
                    ...
                    <ChevronRight className={cn("size-3.5 opacity-55 transition-transform", batchExpanded ? "rotate-90" : "")} />
                </button>
            ) : null}
            {isBatchChild ? (
                <button
...
function ImageInfoBar({ node }: { node: CanvasNodeData }) {
    const w = node.metadata?.naturalWidth || node.width;
    const h = node.metadata?.naturalHeight || node.height;
    const theme = canvasTheme;
    return (
        <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between rounded-b-[inherit] px-2 py-1 text-[10px]" style={{ background: theme.node.panel, color: theme.node.label }}>
            <span>{w}×{h}</span>
        </div>
    );
}
