        ...
        >
            ...
            >
                {nodes.map((node) => {
                    ...
                    const color = node.type === CanvasNodeType.Image ? "#10b981" : node.type === CanvasNodeType.Video ? "#f97316" : node.type === CanvasNodeType.Audio ? "#a855f7" : node.type === CanvasNodeType.Config ? "#60a5fa" : canvasTheme.node.muted;
                    return (
                        <div
                            key={node.id}
                ...
                <div className="pointer-events-none absolute border" style={{ left: viewportRect.x, top: viewportRect.y, width: viewportRect.w, height: viewportRect.h, borderColor: canvasTheme.node.activeStroke, background: `${canvasTheme.node.activeStroke}18` }} />
            </div>
        </div>
    );
}
