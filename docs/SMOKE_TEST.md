...
## 移动端协同（`feature/mobile-collab-optimization`）

视口宽度 `<768px`：

- [ ] 首页底部 dock 可见「热门能力」横向 chips
- [ ] 首页「继续编辑」展示最近会话（已登录）
- [ ] 首页提交 → 过渡 overlay → 创作页画布浮层可见生成进度
- [ ] Job 完成后画布自动居中并高亮最新图
- [ ] 未登录上传 → 登录进创作页后附件仍在 prompt 区
- [ ] 抠图/扩图：未选图有画布顶部提示 → 选图后可成功
- [ ] 长按画布图片出现上下文菜单（抠图/扩图/下载/删除）
- [ ] 文案无「右侧」「Del 删除」等桌面用语
- [ ] 首次进创作页出现 3 步引导（可跳过）
...
- [ ] Studio 侧栏可切换工作区、生成邀请链接
- [ ] 新用户通过 `/join?code=` 可加入团队工作区

需 API 已启动且代码变更后已重启；期望 **25/25** 通过。
