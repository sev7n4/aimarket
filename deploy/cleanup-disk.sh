...
# 可选: KEEP_TAG=<git-sha> GHCR_OWNER=sev7n4 bash deploy/cleanup-disk.sh
set -euo pipefail
...
echo ">>> 删除 AIMarket 旧 GHCR 本地 tag（保留 KEEP_TAG / latest）..."
...
docker system df 2>/dev/null || true
echo ""
echo "完成。可重新部署: cd /opt/aimarket && IMAGE_TAG=<sha> GHCR_OWNER=${GHCR_OWNER} bash deploy/deploy-remote.sh"
