...
**输入来源**：
- 从 `scripts/00_01_workflow_state.json` 获取 `current_fix_cases`
- 分析代码变更影响范围
...
**用例 ID 格式**: 见 `references/05_02_test_case_templates.md`

**测试策略参考**: `references/05_02_test_case_templates.md`

### 5.3 写入状态文件

**写入 `scripts/05_01_test_cases_state.json`**：
...
**注意**：Issue ID 从 `scripts/00_01_workflow_state.json` 获取，不单独生成。

### 5.4 编写失败测试

1. 按 `05_02_test_case_templates.md` 归档格式编写测试代码
2. 运行测试确认失败
3. 更新状态为 `ready`
...
**测试命令参考**: `references/05_02_test_case_templates.md`
...
1. 读取 `scripts/05_01_test_cases_state.json`
2. 按 module 分组
3. 按 `05_02_test_case_templates.md` 归档格式生成文档
...
```bash
# Step 5.3: 写入测试用例状态
jq '.testCases.unit[0].status = "draft"' scripts/05_01_test_cases_state.json > tmp.json && mv tmp.json scripts/05_01_test_cases_state.json

# Step 5.4: 更新为 ready
jq '.testCases.unit[0].status = "ready"' scripts/05_01_test_cases_state.json > tmp.json && mv tmp.json scripts/05_01_test_cases_state.json

# Step 6.3: 更新测试结果
jq '.testCases.unit[0].status = "passed" | .testCases.unit[0].testedAt = "2026-03-22T10:05:00Z"' scripts/05_01_test_cases_state.json > tmp.json && mv tmp.json scripts/05_01_test_cases_state.json

# Step 7.3: 更新归档状态
jq '.testCases.unit[0].status = "archived" | .testCases.unit[0].archivedTo = "assets/test_cases/product/price_cases.md"' scripts/05_01_test_cases_state.json > tmp.json && mv tmp.json scripts/05_01_test_cases_state.json
```
