...
**示例**：
- "undefined reference in timeout" → 代码错误 (undefined)
- "connection refused after syntax error" → 代码错误 (syntax error)
- "assertion failed due to network timeout" → 需求理解错误 (assertion failed)
...
```bash
# 更新重试次数
jq '.retry_count += 1' scripts/00_01_workflow_state.json > tmp.json && mv tmp.json scripts/00_01_workflow_state.json

# 追加错误历史
jq --arg step "10" --arg type "CODE_ERROR" --arg msg "undefined: xxx" --arg ts "2026-03-22T10:00:00Z" \
  '.error_history += [{step: $step, error_type: $type, message: $msg, timestamp: $ts}]' \
  scripts/00_01_workflow_state.json > tmp.json && mv tmp.json scripts/00_01_workflow_state.json

# 重置重试次数（成功后）
jq '.retry_count = 0' scripts/00_01_workflow_state.json > tmp.json && mv tmp.json scripts/00_01_workflow_state.json
```
