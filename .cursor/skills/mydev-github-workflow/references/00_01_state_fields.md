...
> 本文档说明 `scripts/00_01_workflow_state.json` 中各字段的含义和更新时机。
...
| `branching`     | 分支创建中 | Step 2-3 |
| `analyzing`     | 代码分析中 | Step 4   |
| `implementing`  | 代码实现中 | Step 5-7 |
| `verifying`     | 本地验证中 | Step 8   |
| `pushing`       | 代码推送中 | Step 9   |
| `ci_monitoring` | CI监控中 | Step 10  |
| `fixing`        | 错误修复中 | Step 11  |
| `merging`       | PR合并中 | Step 12  |
| `deploying`     | 部署监控中 | Step 13  |
| `completed`     | 已完成   | Step 14  |
...
| Step 12   | `merged: true`          | 合并状态           |

***

## 测试用例状态跟踪

> 用于 `scripts/05_01_test_cases_state.json` 的字段说明
...
| 字段       | 类型          | 说明       | 来源                                        |
| -------- | ----------- | -------- | ----------------------------------------- |
| `id`     | string/null | Issue ID | 从 scripts/00\_01\_workflow\_state.json 获取 |
| `type`   | string/null | 问题类型     | bug/feature/enhancement                   |
| `module` | string/null | 所属模块     | 代码分析推断                                    |
| `branch` | string/null | 分支名称     | 从 scripts/00\_01\_workflow\_state.json 获取 |
...
| 步骤       | 写入内容                               | 说明                                        |
| -------- | ---------------------------------- | ----------------------------------------- |
| Step 5.1 | `currentIssue`                     | 从 scripts/00\_01\_workflow\_state.json 同步 |
...
### 与 scripts/00\_01\_workflow\_state.json 的关系

```
scripts/00_01_workflow_state.json          scripts/05_01_test_cases_state.json
